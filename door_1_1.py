...
while not wifi.isconnected():
    print("Attempting to connect to wifi... please be patient. I'm just a microcontroller")
    time.sleep(1)
...
def Door_Closed_Handler(pin):
    ...
    # If the open limit switch is opened (in other words when the door is moving to the closed state) the irq still has to be handled
...
def Door_Open_Handler(pin):
    ...
    global LS_door_open_state
    global LS_door_closed_state
    # If the closed limit switch is opened (in other words when the door is moving to the open state) the irq still has to be handled
...
async def on_message(topic, msg):
    try:
        # Turning this to blue for the indicator that a message was received
        ...
        if Limit_Switch_Closed.value() == 0 and Limit_Switch_Open.value() == 1:
            ...
            print("Both limit switches are open -- door in an unknown state")
        elif Limit_Switch_Closed.value() == 1 and Limit_Switch_Open.value() == 1:
            current_door_state = "Unknown"
            print("Both limit switches are open -- door in an unknown state")

        # Check if the door is in the desired state already - if yes, don't do anything.
        if new_desired_state == current_door_state:
            print("Door is already in the desired state %s" % current_door_state)
            if current_door_state == "Closed":
                set_neopixel_color("red")
            elif current_door_state == "Open":
                set_neopixel_color("green")
        else:
            # Door is not in the desired state, moving...
            await move_door(new_desired_state)

    except Exception as e:
        print("Error in on_message: {}".format(e))

# Function to turn on the motor and move the door
async def move_door(desired_state):
    global LS_door_closed_state
    global LS_door_open_state
    ...
    print("Checking LS_door_open_state == %i" % LS_door_open_state)
    if desired_state == "Open":
        ...
        Relay_A.value(1)
        Relay_B.value(0)
        # Wait for the door to finish
        try:
            # While the open limit switch is open
            while Limit_Switch_Open.value() == 1:
                print("Waiting for interrupt request from the door open limit switch...")
                time.sleep(3)
        except Exception as e:
        ...
        try:
            ...
            while Limit_Switch_Closed.value() == 1:
                print("Waiting for interrupt request from the door closed limit switch...")
                time.sleep(3)
        except Exception as e:
